...
Your parent types a health question. They get a plain-English answer built only from NHS, NICE and other trusted UK sources, with links to the pages it read and a sheet they can print for the GP.

No app store, no account, no chat history. A password-protected link on the home screen.

**Start here → [DEPLOY.md](DEPLOY.md)** (three levels, from paste-a-prompt to a live page). Just want the prompt? [PROMPT.md](PROMPT.md).
<!-- screenshot: add site/screenshot-phone.png here -->
...
|---|---|---|
| Googling the symptom | first results are ads, forums and US content | reads only NHS, NICE and the official UK medicine leaflets for advice, and Cochrane/PubMed for evidence quality |
...
| Failure | How this page answers it |
|---|---|
...
Triage is different: the 999 / 111 tiers are the model's judgement, not a rule. It can miss an emergency and it can over-call one. It is not a substitute for 999 or 111.
...
**Privacy model:** the question passes through here.now's proxy (which must read it to add the API key), then to Anthropic, which searches the allow-listed sites. Nothing is stored unless the reader presses *Save*, and saved answers live in that browser's `localStorage` only. The site owner can read the feedback box and see here.now's content-blind analytics (countries, approximate visitors — raw IPs are not exposed); the owner does not see questions or answers — the page sends them nowhere the owner can read. The owner controls the page and could change that, so this is a design choice you must trust, not a technical impossibility.

## Model choice

The default is Claude Opus 5 with Anthropic web search. Match the model to the stakes: the more the answer matters, the more capable the model should be. See [MEASUREMENTS.md](MEASUREMENTS.md) for the comparison this is based on.

## Where the data goes — Anthropic's terms, in plain words

The page sends each question through here.now's proxy to Anthropic's API (the searched websites see only the search queries Anthropic makes). What Anthropic does with it, from their own policies as of September 2026 — check the linked pages for the current wording:
...
| **Stored where** | At rest in the **United States** — `"us"` is currently the only workspace geo. Inference runs anywhere by default (`inference_geo: "global"`); you can pin it to the US for 1.1× the price, but there is no EU/UK option. For a UK family this is an international transfer under UK GDPR; Anthropic's [DPA](https://www.anthropic.com/legal/data-processing-addendum) covers it, but it is worth knowing. ([data residency](https://platform.claude.com/docs/en/manage-claude/data-residency)) |
| **Zero data retention** | Available by arrangement with Anthropic (an enterprise contract, not a checkbox). The web-search tool this page uses is ZDR-eligible; a personal Console account will not have it (see Anthropic's [feature eligibility table](https://platform.claude.com/docs/en/manage-claude/api-and-data-retention)). |
| **here.now (the host and proxy)** | Every question passes through here.now's proxy route. Its [privacy policy](https://here.now/privacy) says raw analytics events are kept for "a short debugging window" and does not state a retention period for proxied request bodies; a [DPA](https://here.now/dpa) is published. We have not verified more than that. |
...
- Proxy route rate limit: 20 questions per hour per IP address. A household behind one router shares this limit.
- Feedback: 10 submissions per hour per IP.
- All of this is set in `site/.herenow/proxy.json` and `data.json`.
...
- NHS website content is published under the Open Government Licence v3; NICE content has its own reuse terms. This page links to their pages and quotes short passages; it does not republish them.
- This is a template for a personal, family-scale deployment. It has been used by one family since September 2026. It is not a medical device and is not clinically validated.
