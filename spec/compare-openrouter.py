...
ENGINE = os.environ.get("OR_ENGINE", "exa")          # exa | native (the provider's own search: Anthropic/OpenAI/Google)
ZDR = os.environ.get("OR_ZDR", "1") == "1"
...
if os.environ.get("OR_BARE_DOMAINS"): ALLOWED = [d for d in ALLOWED if "/" not in d]   # OpenAI native search rejects path-scoped domains
...
def ask(q):
    body = json.dumps({"model": MODEL, "provider": {"zdr": ZDR}, "max_tokens": 4000, **({"reasoning": {"effort": os.environ["OR_EFFORT"]}} if os.environ.get("OR_EFFORT") else {}),
        "plugins": [{"id": "web", "engine": ENGINE, "max_results": 6, "include_domains": ALLOWED}],
...
print("MODEL:", MODEL, "| engine:", ENGINE, "| zdr:", ZDR)
...
pathlib.Path(__file__).with_name("compare").joinpath(MODEL.replace("/","_")+"_"+ENGINE+("_zdr" if ZDR else "")+".md").write_text("# "+MODEL+"\n\n"+t+"\n\n## citations\n"+"\n".join(c)+"\n")
print("--- answer head ---\n" + t[:900])
